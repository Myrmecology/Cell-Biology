...
/**
 * Maps each organelle id to a distinct geometric "personality" and material.
 * These are deliberately simplified, stylized shapes rather than biologically
 * literal models — the goal is instant visual recognizability and a cohesive
 * art style across the whole cell, not photorealism.
 */
...
export function OrganelleMesh({ organelle, cellType, isSelected, onSelect }: OrganelleMeshProps) {
  const meshRef = useRef<Mesh>(null)
  const [isHovered, setIsHovered] = useState(false)

  const config = GEOMETRY_CONFIG[organelle.id] ?? DEFAULT_GEOMETRY_CONFIG

  const position = organelle.scenePosition[cellType]
  const baseScale = organelle.sceneScale?.[cellType] ?? 1

  // Target scale grows subtly on hover/select for clear interactive feedback.
  ...
  // Smoothly animate toward the target scale each frame, and give every
  // organelle a very slight idle drift so the cell feels alive, not static.
  useFrame((state, delta) => {
    ...
    meshRef.current.position.y = (position?.y ?? 0) + Math.sin(t + (position?.x ?? 0)) * 0.03
  })
